...
const PrinterModal = memo(({ btPermission}) => {
  ...
  const checkPrinter = useCallback(async () => {
    ...
    if(Platform.OS === 'android') {
      try {
        printerAddress = await AsyncStorage.getItem('printerAddress')
        printerName = await AsyncStorage.getItem('printerName')
      } catch (error) {
        console.log(error)
      }
  ...
  const setPrinterSStore = async (row) => {
    try {
      await AsyncStorage.setItem('printerAddress', row.address)
      await AsyncStorage.setItem('printerName', row.name)
    } catch (error) {
